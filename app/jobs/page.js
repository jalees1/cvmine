...
import React, { useState } from 'react';
import { Search, MapPin, Clock, DollarSign, Briefcase, Users } from 'lucide-react';
import jobsData from '@/data/jobsdata'; // Adjust the path as needed
...
const JobListingInterface = () => {
  ...
  // Use the imported job data instead of mock data
  const jobs = jobsData;
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="md:w-3/4">
          <h2 className="text-xl font-bold mb-4">Showing {filteredJobs.length} Results</h2>
            ...
            <div 
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-bold">{job.title}</h3>
                    <Link href={`/details?company=${job.company}&title=${job.title}&experience=${job.experience}&salary=${job.salary}&type=${job.type}&location=${job.location}&posted=${job.posted}`}>
                      <p className="text-gray-600 cursor-pointer hover:underline">{job.company}</p>
                    </Link>
                  </div>
                </div>
                <span className="bg-orange-200 text-orange-800 px-2 py-1 rounded text-sm">Job</span>
